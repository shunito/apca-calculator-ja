<script>
    ...
    export const fontWeightList = [ "100","200","300","400","500","600","700","800","900" ];

    export let textColor = "#333333";
    export let backgroundColor = "#eeeeee";
    export let sampleText = "あのイーハトーヴォのすきとおった風、夏でも底に冷たさをもつ青いそら、うつくしい森で飾られたモリーオ市、郊外のぎらぎらひかる草の波。 またそのなかでいっしょになったたくさんのひとたち、ファゼーロとロザーロ、羊飼のミーロや、顔の赤いこどもたち、地主のテーモ、山猫博士のボーガント・デストゥパーゴなど、いまこの暗い巨きな石の建物のなかで考えていると、みんなむかし風のなつかしい青い幻燈のように思われます。";
    ...
    const calculator = () => {
        const result = lcTable.filter( ( value ) =>{ return value.px === fontSize } );
        if(result.length > 0){
...
<section>
        ...
        <div class="grid color-selector">
            <div>
                ...
                <div class="grid color-set">
                    <input type="text" id="textColor" name="textColor" bind:value={textColor} on:blur={handleColorChange}>       
                    <input type="color" id="textColorPicker" name="color" bind:value={textColor} aria-label="テキスト カラーピッカー" class="color-picker" on:change={handleColorChange}>
                </div>
            </div>
            <button type="button" class="btn-swap secondary" on:click={handleSwapColor}>入替</button>
            <div>
                <label for="backgroundColor">バックグラウンドカラー</label>
                <div class="grid color-set">
                    <input type="text" id="backgroundColor" name="textColor" bind:value={backgroundColor} on:blur={handleColorChange}>       
                    <input type="color" id="backgroundColorPicker" name="color" bind:value={backgroundColor} aria-label="バックグラウンド カラーピッカー" class="color-picker" on:change={handleColorChange}>
                </div>
            </div>
        </div>
        <div>
            <label for="sampleText">サンプル文章</label>
            <textarea id="sampleText" class="sampleTextArea" on:change={handleChangeSampleText}>{sampleText}</textarea>
        </div>
    ...
    <div class="result">
        <h3>チェック結果</h3>
        <ul>
            <li>APCAコントラスト <strong>{contrastLc.toFixed(1)}</strong> <small>Lc</small> （Rating : {APCAresult.Rating}）</li>
            {#if APCAresult.Lc == "none"}<li>推奨されないフォント設定です。</li>
            {:else}
                <li>推奨コントラスト <strong>{APCAresult.Lc}</strong> <small>Lc</small>
                {#if Math.abs(contrastLc) > APCAresult.Lc }（基準値クリア）{:else}（コントラスト不足）{/if}
                </li>
            {/if}
            ...
            {#if APCAresult.Add15BlocksText }<li>本文で利用する場合には推奨コントラストに15追加した値を設定してください。
